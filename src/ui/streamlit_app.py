...
from ..models.registry import detect_run_method

st.set_page_config(page_title='Gauge-QA Demo', page_icon='🧭', layout='wide')
st.title('Gauge-QA Demo')
if 'pipeline' not in st.session_state:
    st.session_state.pipeline = None
if 'loaded_run_dir' not in st.session_state:
    st.session_state.loaded_run_dir = None
    ...
    if st.button('加载模型'):
        try:
            with st.spinner('正在加载模型...'):
                method = detect_run_method(run_dir)
                pipe = UnifiedQAPipeline(run_dir, device=device)
            st.session_state.pipeline = pipe
            st.session_state.loaded_run_dir = run_dir
            st.session_state.loaded_method = method
            st.success(f'加载成功：{method}')
...
if st.button('生成答案'):
    if st.session_state.pipeline is None:
        ...
        st.write(st.session_state.pipeline.answer(context, question))
